...
One later correction, which leaves this decision intact. "It holds `@types/react@19.2.17`" above was true when measured, but it is not guaranteed: the slot is chosen by graph-traversal order rather than by the lockfile, and it held **18** on two CI runs — breaking the frontend the same way 19 broke the admin. See `0012-react-types-resolve-from-the-root.md`, which stops pnpm creating the slot at all. The `paths` pin recorded here is still required and still correct.
